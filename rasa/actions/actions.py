...
class ActionCheckAndPrint(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if not is_sorting_valid(srtng):
            ...
            return []
        try:
            suggestions = make_api_request(csn,ispn,srtng)        
...
def make_api_request(cuisine, is_open,sorting):
    api_key = "AWeaxSiD-XP8CScdutIZzKYUwRm7ArmiYdq8uZXEBI1jCTvLDCKFFCrd3WXmam_UANrvnkrEgfNusvv6zpUxsPo-76DTNHASgFc3bHAFzYBna27aNNRUf0KL9KhBZXYx"
    ...
    for i in business_data['businesses']:
        suggestions_list.append(
            "Restaurant " + i['name'] + " with rating " + str(i['rating']) + " and price range " + i['price'] + " is located at " + i['location']['address1'] + ".")
